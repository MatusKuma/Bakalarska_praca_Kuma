...
def exec_trading(df, my_Account, trade_file, stat_file, trade_file_req_data, trading_fee_rate, date_from, date_to):
    ...
    for index, row in df.iterrows():
        ...
        if rsi >= overbought:
            if trade_is_open:
                if trades[(trade_number * 2) - 2]['action'] == "SHORT":
                    balance.append(my_Account.quote_balance * (open_price/last_price))
                    last_price = open_price
                    continue
                close_trade_info = close_trade(unix, trades, trade_number, open_price, my_Account, trading_fee_rate, date, high_price, low_price, close_price)
                balance.append(my_Account.quote_balance)
                trades.append(close_trade_info)
                print_trade_info(close_trade_info)
                trade_number += 1
                trade_info = open_trade(unix, trade_number, open_price, my_Account, 100, trading_fee_rate, 2, date, high_price, low_price, close_price)
        ...
        if rsi <= oversold:
            if trade_is_open:
                if trades[(trade_number * 2) - 2]['action'] == "LONG":
